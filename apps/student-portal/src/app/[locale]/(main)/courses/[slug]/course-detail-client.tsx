...
interface CourseDetailClientProps {
  slug: string;
  // Dữ liệu fetch sẵn ở Server Component → seed vào TanStack để render nội dung
  // thật ngay từ HTML server (SEO), không cần fetch lại ở client lần đầu.
  initialCourse: { data: ApiCourse };
}
...
export function CourseDetailClient({ slug, initialCourse }: CourseDetailClientProps) {
  ...
  const { data: courseData } = useCourseDetail(slug, initialCourse);
              ...
              <TabsContent value="overview">
                ...
                {course.description && (
                  <div className="mb-6">
                    ...
                    <div
                      className="prose prose-sm dark:prose-invert prose-p:text-foreground prose-li:text-foreground prose-headings:text-foreground max-w-none break-all"
